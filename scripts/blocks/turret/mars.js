const elib = require("mindustrinity/effectlib");
const bulletLib = require("mindustrinity/bulletlib");
const prox = require("mindustrinity/prox-block-lib");
const trailLib = require("mindustrinity/traillib");
...
const mars = extendContent(ItemTurret, "mars", {
	init(){
		...
		this.amplifier = Vars.content.getByName(ContentType.block, "mindustrinity-power-amplifier");
		this.super$init();
	},
	onDestroyed(tile){
	...
	load(){
		this.super$load();
		this.baseRegion = Core.atlas.find("mindustrinity-block-8");
		this.region = Core.atlas.find(this.name);
	},
	generateIcons(){
		return [
			Core.atlas.find("mindustrinity-block-8"),
